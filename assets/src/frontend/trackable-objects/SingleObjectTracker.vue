<template>
	<div class="stackonet-dashboard-tracker">
		<template v-if="Object.keys(object).length">
			<side-navigation :active="true" :show-overlay="false" nav-width="400px" class="tracker-side-navigation">
				<map-object-card
				...
				>
					<div class="card__actions" style="display: flex;">
						...
						<div style="display: none">
							...
							<mdl-button type="raised" color="primary" v-if="!useSnapToRoads"
										@click="lineType('optimised')">
								Optimised
							</mdl-button>
						</div>
					</div>
...
<script>
	...
	export default {
		...
		mixins: [TrackerMixin],
		components: {MdlButton, MapObjectCard, FlatPickr, sideNavigation},
		data() {
			return {
