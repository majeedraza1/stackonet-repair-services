<template>
	<div>
		<h1 class="wp-heading-inline">Carrier Store</h1>
		<div class="clear"></div>
		...
		<modal :active="has_active_item" title="Survey Info" @close="activeItem = {}">
			...
			<list-item label="Name">{{activeItem.name}}</list-item>
			<list-item label="Store">{{activeItem.store}}</list-item>
			<list-item label="Industry">{{activeItem.industry}}</list-item>
	...
	export default {
		...
		methods: {
			getItems() {
				let self = this;
				axios
					.get(stackonetSettings.root + `/carrier-store?status=${self.status}&paged=${self.currentPage}`)
					.then((response) => {
			...
			trashAction(item, action) {
				...
				self.loading = true;
				axios
					.post(stackonetSettings.root + '/carrier-store/delete', {
						id: item.id,
						action: action
					})
					.then((response) => {
			...
			batchTrashAction(ids, action) {
				...
				self.loading = true;
				axios
					.post(stackonetSettings.root + '/carrier-store/batch_delete', {
						ids: ids,
						action: action
					})
					.then((response) => {
