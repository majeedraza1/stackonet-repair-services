...
class UnsupportedAreaController extends ApiController {
	...
	public function register_routes() {
		register_rest_route( $this->namespace, '/unsupported_areas', [
			[
		...
		register_rest_route( $this->namespace, '/unsupported_areas/batch/trash', [
			'methods'  => \WP_REST_Server::DELETABLE,
			'callback' => [ $this, 'trash_items' ],
		] );

		register_rest_route( $this->namespace, '/unsupported_areas/batch/restore', [
			'methods'  => \WP_REST_Server::DELETABLE,
			'callback' => [ $this, 'restore_items' ],
		] );

		register_rest_route( $this->namespace, '/unsupported_areas/batch/delete', [
			'methods'  => \WP_REST_Server::DELETABLE,
			'callback' => [ $this, 'delete_items' ],
		] );

		register_rest_route( $this->namespace, '/unsupported_areas/(?P<id>\d+)', [
	...
	public function get_items( $request ) {
		...
		$areas  = ( new UnsupportedArea() )->find( $args );
		$counts = ( new UnsupportedArea() )->count_records();

		return $this->respondOK( [ 'items' => $areas, 'counts' => $counts ] );
	}
	...
	public function update_item( $request ) {
		...
		$area = ( new UnsupportedArea() )->find_by_id( $id );
		if ( ! $area instanceof UnsupportedArea ) {
	...
	public function restore_items( $request ) {
		...
		$ids = array_map( 'intval', $ids );
		foreach ( $ids as $id ) {
			( new UnsupportedArea() )->restore( $id );
		}
